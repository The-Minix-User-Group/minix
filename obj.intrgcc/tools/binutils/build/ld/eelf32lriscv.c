...
static lang_output_section_statement_type *gldelf32lriscv_place_orphan
  (asection *, const char *, int);

static void
gldelf32lriscv_map_segments (bfd_boolean need_layout)
{
  int tries = 10;
...
gldelf32lriscv_after_allocation (void)
{
  ...
  gldelf32lriscv_map_segments (need_layout);
}
...
static bfd_boolean
gldelf32lriscv_load_symbols (lang_input_statement_type *entry)
{
  int link_class = 0;
...
static void
gldelf32lriscv_vercheck (lang_input_statement_type *s)
{
  const char *soname;
...
static void
gldelf32lriscv_stat_needed (lang_input_statement_type *s)
{
  struct stat st;
  const char *suffix;
  const char *soname;
...
static bfd_boolean
gldelf32lriscv_try_needed (struct dt_needed *needed,
				 int force)
{
  bfd *abfd;
  ...
  if (! force)
      ...
      if (needs != NULL)
	{
	  ...
	  lang_for_each_input_file (gldelf32lriscv_vercheck);
	  if (global_vercheck_failed)
	    {
	      bfd_close (abfd);
  ...
  global_found = NULL;
  lang_for_each_input_file (gldelf32lriscv_stat_needed);
  if (global_found != NULL)
    {
...
static bfd_boolean
gldelf32lriscv_search_needed (const char *path,
				    struct dt_needed *n, int force, int prepend_sysroot)
{
  ...
  if (name[0] == '/') {
    if (prepend_sysroot && ld_sysroot) {
      ...
      rv = gldelf32lriscv_try_needed (&needed, force);
      free(filename);
      return rv;
    }
    return gldelf32lriscv_try_needed (n, force);
  }
  ...
  while (1)
      ...
      else
	{
      ...
      needed.name = filename;
      if (gldelf32lriscv_try_needed (&needed, force))
	return TRUE;
...
static char *
gldelf32lriscv_add_sysroot (const char *path)
{
  int len, colons, i;
  char *ret, *p;
...
static void
gldelf32lriscv_check_needed (lang_input_statement_type *s)
{
  const char *soname;
...
gldelf32lriscv_id_note_section_size (bfd *abfd,
...
static bfd_boolean
gldelf32lriscv_write_build_id_section (bfd *abfd)
{
...
gldelf32lriscv_after_open (void)
{
  ...
  if (link_info.emit_note_gnu_build_id)
	...
	{
	  size = gldelf32lriscv_id_note_section_size (abfd, &link_info);
	  if (size == 0)
	    {
	      ...
	      if (s != NULL && bfd_set_section_alignment (abfd, s, 2))
		{
		  ...
		  t->after_write_object_contents
		    = &gldelf32lriscv_write_build_id_section;
  ...
  for (l = needed; l != NULL; l = l->next)
	...
	continue;
      ...
      global_needed = l;
      global_found = NULL;
      lang_for_each_input_file (gldelf32lriscv_check_needed);
      if (global_found != NULL
	  && (bfd_elf_get_dyn_lib_class (global_found->the_bfd)
	      & DYN_AS_NEEDED) == 0)
	continue;
      ...
      if (global_found != NULL)
	{
	  ...
	  if (gldelf32lriscv_try_needed (&nn, TRUE))
	    continue;
	}
	 ...
	 their use.  See gldelf32lriscv_vercheck comment.  */
      for (force = 0; force < 2; force++)
	{
	  ...
	  if (gldelf32lriscv_search_needed (command_line.rpath_link,
						  &n, force, 0))
	    break;
	  if (gldelf32lriscv_search_needed (command_line.rpath,
						  &n, force, 1))
	    break;
	  found = 0;
	  ...
	  for (; !found && rp != NULL; rp = rp->next)
	    {
	      found = (rp->by == l->by
		       && gldelf32lriscv_search_needed (rp->name,
							      &n,
							      force, 1));
	    }
	  if (found)
	  ...
	  for (search = search_head; search != NULL; search = search->next)
		...
		continue;
	      ...
	      nn.name = filename;
	      if (gldelf32lriscv_try_needed (&nn, force))
		break;
	      free (filename);
	    }
	  if (search != NULL)
...
static void
gldelf32lriscv_find_exp_assignment (etree_type *exp)
{
  ...
  switch (exp->type.node_class)
      ...
      if (strcmp (exp->assign.dst, ".") != 0)
	{
      ...
      gldelf32lriscv_find_exp_assignment (exp->assign.src);
      break;

    case etree_binary:
      gldelf32lriscv_find_exp_assignment (exp->binary.lhs);
      gldelf32lriscv_find_exp_assignment (exp->binary.rhs);
      break;

    case etree_trinary:
      gldelf32lriscv_find_exp_assignment (exp->trinary.cond);
      gldelf32lriscv_find_exp_assignment (exp->trinary.lhs);
      gldelf32lriscv_find_exp_assignment (exp->trinary.rhs);
      break;

    case etree_unary:
      gldelf32lriscv_find_exp_assignment (exp->unary.child);
      break;
...
static void
gldelf32lriscv_find_statement_assignment (lang_statement_union_type *s)
{
  if (s->header.type == lang_assignment_statement_enum)
    gldelf32lriscv_find_exp_assignment (s->assignment_statement.exp);
}


/* used by before_allocation and handle_option. */
static void
gldelf32lriscv_append_to_separated_string (char **to, char *op_arg)
{
  if (*to == NULL)
    *to = xstrdup (op_arg);
  else
...
gldelf32lriscv_before_allocation (void)
{
  ...
  lang_for_each_statement (gldelf32lriscv_find_statement_assignment);
	...
	if (audit_libs && *audit_libs != '\0')
	  {
		...
		if (cp != NULL && *cp != '\0')
		  gldelf32lriscv_append_to_separated_string (&depaudit, cp);
...
static bfd_boolean
gldelf32lriscv_open_dynamic_archive
...
static lang_output_section_statement_type *
gldelf32lriscv_place_orphan (asection *s,
				   const char *secname,
				   int constraint)
{
...
gldelf32lriscv_handle_option (int optc)
{
  switch (optc)
    ...
    case OPTION_AUDIT:
	gldelf32lriscv_append_to_separated_string (&audit, optarg);
	break;

    case 'P':
	gldelf32lriscv_append_to_separated_string (&depaudit, optarg);
	break;
