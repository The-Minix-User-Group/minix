...
{
  gcc_assert (IN_RANGE (opts->x_riscv_lra_flag, -128, 127));

  if (targetm.target_option.save)
    targetm.target_option.save (ptr);

  ptr->x_riscv_lra_flag = opts->x_riscv_lra_flag;
}
...
{
  opts->x_riscv_lra_flag = ptr->x_riscv_lra_flag;
...
{
  fputs ("\n", file);
  if (ptr->x_riscv_lra_flag)
    fprintf (file, "%*s%s (%#x)\n",
             indent, "",
             "riscv_lra_flag",
             ptr->x_riscv_lra_flag);
